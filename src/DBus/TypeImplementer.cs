...
namespace DBus
{
	...
	class TypeImplementer
	{
		...
		{
			if (canSave)
				throw new Exception("Assembly Saving is unsupported using .NET Core");

			asmB = AssemblyBuilder.DefineDynamicAssembly (new AssemblyName (name), AssemblyBuilderAccess.Run);
			// asmB = Assembly.Builder.DefinteDynamicAssembly (new AssemblyName (name), canSave ? AssemblyBuilderAccess.RunAndSave : AssemblyBuilderAccess.Run);
